...
c=n d=n h=n i=n I=n l=n r=n s=n u=n U=n w=n v=n W=n K=n C=n BRANCH=$CHATWOOT_BRANCH SERVICE=web DEPLOYMENT_TYPE=full CONVERT_TO=""
...
if [ "$d" == "y" ]; then
  echo "console: $c, debug: $d, help: $h, install: $i, Install: $I, BRANCH: $CHATWOOT_BRANCH, \
...
function setup_chatwoot() {
  ...
  git clone "$CHATWOOT_REPO"
  cd chatwoot
  git checkout "$CHATWOOT_BRANCH"
  bundle
  pnpm i
...
function configure_systemd_services() {
  ...
  if [ "$DEPLOYMENT_TYPE" == "web" ]; then
    ...
    cp /home/chatwoot/chatwoot/integration/chatwoot-web.1.service /etc/systemd/system/chatwoot-web.1.service
    cp /home/chatwoot/chatwoot/integration/chatwoot-web.target /etc/systemd/system/chatwoot-web.target
    ...
    cp /home/chatwoot/chatwoot/integration/chatwoot-worker.1.service /etc/systemd/system/chatwoot-worker.1.service
    cp /home/chatwoot/chatwoot/integration/chatwoot-worker.target /etc/systemd/system/chatwoot-worker.target
    ...
    cp /home/chatwoot/chatwoot/integration/chatwoot-web.1.service /etc/systemd/system/chatwoot-web.1.service
    cp /home/chatwoot/chatwoot/integration/chatwoot-worker.1.service /etc/systemd/system/chatwoot-worker.1.service
    cp /home/chatwoot/chatwoot/integration/chatwoot.target /etc/systemd/system/chatwoot.target
  ...
  cp /home/chatwoot/chatwoot/integration/chatwoot /etc/sudoers.d/chatwoot
  cp /home/chatwoot/chatwoot/integration/setup /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget "${CHATWOOT_RAW_BASE}/${CHATWOOT_BRANCH}/integration/nginx_chatwoot.conf"
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Chatwoot to v$CW_VERSION (branch: $CHATWOOT_BRANCH)"

  # Warning for non-master branch upgrades
  if [ "$CHATWOOT_BRANCH" != "main" ]; then
    cat << EOF
...
EOF
  ...
  git fetch
  git checkout "$CHATWOOT_BRANCH" && git pull
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="${CHATWOOT_RAW_BASE}/${CHATWOOT_BRANCH}/VERSION_CWCTL"
